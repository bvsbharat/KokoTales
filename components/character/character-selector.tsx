...
import { Upload, X, Check, ArrowLeft, ArrowRight, User, Camera } from "lucide-react"
...
function CharacterCard({ 
  ...
  return (
    ...
    >
      <Card className={cn(
        "comic-character-card h-80 cursor-pointer transition-all duration-300",
        ...
        <CardContent className="p-6 h-full flex flex-col">
          ...
          <div 
            {...getRootProps()} 
            className={cn(
              "flex-1 border-4 border-dashed border-gray-400 rounded-none flex flex-col items-center justify-center mb-4 transition-colors",
            ...
            {character.uploadedImage ? (
              <div className="relative w-full h-full">
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation()
                    onUpdate({ uploadedImage: undefined, base64Image: undefined })
                  }}
                  className="absolute top-2 right-2 bg-comic-red text-white border-2 border-black p-1 hover:bg-red-600"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
